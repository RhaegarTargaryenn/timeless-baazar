...
import { HiMail, HiLockClosed, HiUser, HiEye, HiEyeOff, HiArrowLeft, HiCheckCircle } from 'react-icons/hi';
...
const Signup = () => {
  ...
  const [verifyEmail, setVerifyEmail] = useState(''); // shows spam notice when set

  // ── Email Signup ──
  const handleEmailSignup = async (e) => {
    e.preventDefault();
    if (emailForm.password !== emailForm.confirmPassword) { toast.error('Passwords do not match!'); return; }
    if (emailForm.password.length < 6) { toast.error('Password must be at least 6 characters!'); return; }
    setLoading(true);
    try {
      ...
      await setDoc(doc(db, 'users', userCredential.user.uid), {
        name: emailForm.name, email: emailForm.email,
        createdAt: new Date().toISOString(), emailVerified: false, orders: [],
      });
      await sendEmailVerification(userCredential.user, { url: window.location.origin + '/login', handleCodeInApp: false });
      await auth.signOut();
      setVerifyEmail(emailForm.email);
    } catch (err) {
      if (err.code === 'auth/email-already-in-use') toast.error('Email already in use. Log in instead!');
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 flex items-center justify-center py-10 px-4">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-sm">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-8 text-center">
            ...
            <p className="text-sm font-semibold text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-700 rounded-xl px-4 py-2 mb-5 break-all">
              {verifyEmail}
            </p>
            ...
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4 text-left mb-6">
              <p className="text-xs font-bold text-amber-800 dark:text-amber-300 mb-1">📂 Email not in inbox?</p>
              <p className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                Check your <span className="font-bold">Spam / Junk</span> folder. Emails from
                {' '}<span className="font-mono text-xs">noreply@...</span> sometimes land there.
                Mark it <span className="font-bold">Not Spam</span> so future emails arrive in inbox.
              </p>
            </div>
            ...
            <motion.button
              ...
              className="w-full py-2.5 bg-gradient-to-r from-green-500 to-green-600 text-white font-semibold rounded-xl shadow-md text-sm"
            >
              Go to Sign In
            </motion.button>
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 flex items-center justify-center py-10 px-4">
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-sm">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Join Timeless Baazar today</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700">
          <div className="p-6 space-y-4">
            <form onSubmit={handleEmailSignup} className="space-y-3.5">
                  {[
                    { label: 'Full Name', name: 'name', type: 'text', icon: <HiUser className="w-4 h-4" />, placeholder: 'Your Name', autocomplete: 'name' },
                    { label: 'Email', name: 'email', type: 'email', icon: <HiMail className="w-4 h-4" />, placeholder: 'you@example.com', autocomplete: 'email' },
                    ...
                    <div key={name}>
                      ...
                      <div className="relative">
                        ...
                        <input type={type} value={emailForm[name]} onChange={(e) => setEmailForm({ ...emailForm, [name]: e.target.value })} required autoComplete={autocomplete}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type={showPassword ? 'text' : 'password'} value={emailForm.password} onChange={(e) => setEmailForm({ ...emailForm, password: e.target.value })} required minLength={6} autoComplete="new-password"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <HiLockClosed className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4" />
                      <input type={showConfirm ? 'text' : 'password'} value={emailForm.confirmPassword} onChange={(e) => setEmailForm({ ...emailForm, confirmPassword: e.target.value })} required autoComplete="new-password"
            ...
            <p className="text-center text-sm text-gray-500 dark:text-gray-400">
              Have an account? <Link to="/login" className="text-green-600 dark:text-green-400 font-semibold hover:underline">Sign in</Link>
            </p>
          </div>
        </div>
